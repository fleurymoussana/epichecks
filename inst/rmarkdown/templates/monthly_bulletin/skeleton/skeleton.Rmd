...
## set default NA to -
## stop scientific notation (relevant for big numbers)
options(knitr.kable.NA = "-", 
        scipen = 999)

## Installing required packages for this template
required_packages <- c("aweek",      # converting between epiweeks and dates
                       ...
                       "matchmaker", # for recoding variables based on a dictionar
...
input_path_phe <- here::here("Data Files", "2017_2019_PHE_31_10_2019.xlsx") 
...
top_triggers_countries <- threshold_counts %>% 
  tab_linelist(disease, strata = country, row_total = TRUE) %>% 
  select(-contains("proportion")) %>%  # drop proportion columns
  mutate(countries = rowSums(select(., contains("n")) != 0)) %>% # count countries (sum non-zeros)
  arrange(desc(Total)) %>%   # sort based on decreasing counts
  top_n(5, Total)


## pull together a sentence listing events if there were any
top_triggers_list <- str_glue(" {top_triggers_countries$value} ({top_triggers_countries$Total} events in {top_triggers_countries$countries} countries)")
...
expected reports. 
+ The weekly reporting completeness was `r wkly_completeness`. 
...
+ A total of `r nrow(threshold_counts)` alert thresholds were triggered during 
...
+ The top 5 diseases with alert thresholds triggered are: `r top_triggers_list` 
+ `r count_ongoing_events` outbreaks and other health emergencies are ongoing in
the region in `r count_ongoing_events_countries`. 
+ The top 5 ongoing events are: `r highlights_events_list` 
...
## get counts of submitted reports and complete (non-missing reports) 
## and proportion of the expected reports submitted for the month (i.e. how many weeks)
completeness_data <- processed_data_agg %>% 
  group_by(country, epiweek) %>% 
  ## get number of reports (diseases*districts)
  ## get number of retrpos (diseases*districts) where no missing flags
  summarise(reports  = n(), 
            complete = sum(is.na(miss_comment))) %>% 
  ## fill in zeros for missing epiweeks
  complete(epiweek, fill = list(reports = 0, 
                                complete = 0)) %>% 
  ## count reports where diseases*districts is not zero (i.e. submitions)
  ## count the number of reports where non missing diseases*districts is same as total disease*district
  ## divide submitted reports by expected weekly reports for proportion
  summarise(tot_reps = sum(reports != 0), 
            complete_reps = sum(complete == reports), 
            prop_reps = tot_reps / expected_reports * 100)


## join data set to the shapefile for plotting 
afro_shp <- left_join(afro_shp, completeness_data, by = "country")

## recode NA props to zero for AFRO countries 
## recode 0 counts to NA for all countries 
afro_shp <- afro_shp %>% 
  mutate(prop_reps = if_else(is.na(prop_reps) &
                                   WHO_REG == "AFRO", 0,
                                 prop_reps),
         tot_reps = if_else(tot_reps == 0, NA_integer_, tot_reps), 
         complete_reps = if_else(complete_reps == 0, NA_integer_, complete_reps)
         )
...
overall_plot <- ggplot() +
  ...
  scale_colour_manual(values = c("#8b0000", "#F7C252"), 
                      name = "Report type") +
...
**Figure 4**: Tile plot showing number of country reports by completeness 
...
## need one more colour then the expected reports to include 0 counts
cols2 <- make_colours(num1 = expected_reports + 1, 
                      num2 = expected_reports + 1)
...
## create counts of timley and complete reports by country
## create the same uniqueID by combining completeness & timeliness
...
legend <- ggplot(
  ...
  scale_fill_identity() + 
  ## label the axis ticks from 0 to the number of reports
  scale_x_continuous(breaks = c(0:expected_reports)) +
  scale_y_continuous(breaks = c(0:expected_reports)) +
  ## make tiles boxes
  coord_fixed() + 
  ## label axes 
...
base_map + legend + wrap_plots(island_plot, nrow = 1) + plot_layout(design = layout)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 This section plots a map for thresholds - a choropleth which is coloured depending 
 on number of thresholds triggered, and dots proportional to number of diseases
 triggering
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
...
## use all reports to get counts of thresholds triggered and diseases trigerring
## thresholds by country and week
threshold_map_counts <- processed_data_agg %>% 
  group_by(epiweek, country) %>%                 # do for each week and each country
  filter(alert == "Flag") %>%                    # only keep rows over thresholds
  summarise(threshold_counts = n(),              # count total thresholds triggered
            disease_counts = n_distinct(disease)  # count diseases over threshold
            ) %>% 
  ungroup() %>% 
  mutate(epiweek = str_replace(epiweek, "-", "_"))

## spread to wide format - one column per country
threshold_map_counts <- pivot_wider(threshold_map_counts, 
                                    id_cols = country, 
                                    names_from = epiweek, 
                                    values_from = c(threshold_counts, disease_counts))
## bind counts columns to shapefile
afro_shp <- left_join(afro_shp, threshold_map_counts, by = "country")

## swap the dash for the underscore to work with ggplot
loop_weeks <- str_replace(levels(current_weeks), "-", "_")

## define breaks in advance 

for (i in loop_weeks) {
  
  ## pull together name of threshold count variable for week
  thresh_var <- str_c("threshold_counts_", i)
  
  ## create categorical variable out of threshold_counts for week
  afro_shp[[thresh_var]] <- cut(as.numeric(afro_shp[[thresh_var]]),
           breaks = c(0, 1, 100, 200, 300, 400, 500, Inf),
           include.lowest = TRUE,
           labels = c("0", "1-100", "101-200", "201-300", "301-400", "401-500", ">500")
           )
  
  ## change countries with threshold count NA who are in AFRO to 0 
  afro_shp[[thresh_var]][is.na(afro_shp[[thresh_var]]) & afro_shp$WHO_REG == "AFRO"] <- "0"

  ## pull together name of disease count variable for week 
  disease_var <- str_c("disease_counts_", i)
  
  ## create categorical variable out of threshold_counts for week
  afro_shp[[disease_var]] <- cut(as.numeric(afro_shp[[disease_var]]),
           breaks = c(0, 1, 2, 3, 4, Inf),
           include.lowest = FALSE,
           labels = c(1, 2, 3, 4, 5)
           )
  
  ## change to an integer for plotting 
  afro_shp[[disease_var]] <- as.integer(afro_shp[[disease_var]])
  
  ## change countries with threshold count NA who are in AFRO to 0 
  # afro_shp[[disease_var]][is.na(afro_shp[[disease_var]]) & 
  #                           afro_shp$WHO_REG == "AFRO"] <- "0"
  ...
  overall_plot <- ggplot() +
    ...
    scale_fill_discrete_sequential(drop = FALSE, 
                      palette = "Oranges", 
                      na.value = "Grey60",
                      labels = c(levels(afro_shp[[thresh_var]]), 
                                 "Non-AFRO"),
                      name = "Threshold alerts (n)") +
    ## plot dots for total cases - size based on number
    geom_point(data = afro_shp, 
               aes_string(size = disease_var, 
                    geometry = "geometry"), 
                   colour =  "#79ABE2",
               stat = "sf_coordinates") +
    ## set the range in size of points
    scale_size_continuous(range = c(2,8), 
                          name = "Disease types (n)",
                          limits = c(1, 5),
                          labels = c("1", "2", "3", "4", "5+")) +
    ## add a scale bar
    annotation_scale(location = "br") +
    ...
    theme_void(base_size = 18) + 
    ## add a title 
    labs(title = str_c("Calendar week ", str_replace(i, "_", " ")))
...
**Figure 7**: Distribution of ongoing public health events in the African region
as of `r format(aweek::week2date(end_week) + 7, "%d/%m/%Y")`
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// events_plot \\\
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Among the `r nrow(outbreak_data)` ongoing outbreaks in the African region, 
`r fmt_count(outbreak_data, outbreak == "Cholera")` are due to cholera outbreaks. 
`r length(unique(cholera_outbreak_data$country))` countries 
are experiencing a cholera outbreak: `r cholera_countries`. 

During `r str_glue("{current_month} {current_year}")`, 
`r cholera_counts$cases_total` cases have been reported 
(`r cholera_counts$cases_susp_prob` suspected and `r cholera_counts$cases_conf`
confirmed) including `r cholera_counts$deaths` deaths
(case fatality ratio : `r cholera_counts$CFR`%). 

**Figure 9**: Distribution of cases and CFR by week for 
...
ggplot(cholera_weekly_country, aes(x = epiweek, y = cases_total, fill = reorder(country, cases_total))) + 
  geom_bar(stat = "identity") + 
...
**Figure 11**: Spatial distribution of cumulative cholera cases and CFR by country
in the African region as of `r format(aweek::week2date(end_week) + 7, "%d/%m/%Y")`

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// cholera_map \\\
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 
 This section plots a map for countries experiencing a cholera outbreak, including
 dots for cases and deaths as well as chloropleth for CFR
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 This section calculates counts and proportions for indicators used in 
 reporting events that are occuring due to Measles based on outbreak linelist
 data. 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 

```{r measles_events}

## subset outbreak data set for Measles
measles_outbreak_data <- outbreak_data %>% 
  filter(outbreak == "Measles") 

## pull together countries experiencing measles outbreak
measles_countries <- str_c(
  unique(measles_outbreak_data$country),
  collapse = ", ")

## get summary counts for african region 
measles_counts <- measles_outbreak_data %>% 
  summarise(cases_total = sum(new_suspected_cases, na.rm = TRUE) + 
              sum(new_probable_cases, na.rm = TRUE) + 
              sum(new_confirmed_cases, na.rm = TRUE), 
...
Among the `r nrow(outbreak_data)` ongoing outbreaks in the African region, 
`r fmt_count(outbreak_data, outbreak == "Measles")` are due to measles outbreaks. 
`r length(unique(measles_outbreak_data$country))` countries 
are experiencing a measles outbreak: `r measles_countries`. 

During `r str_glue("{current_month} {current_year}")`, 
`r measles_counts$cases_total` cases have been reported 
(`r measles_counts$cases_susp_prob` suspected and `r measles_counts$cases_conf` 
confirmed) including `r measles_counts$deaths` deaths
(case fatality ratio : `r measles_counts$CFR`%). 

**Figure 12**: Distribution of cases and CFR by week for 
`r length(unique(measles_outbreak_data$country))` countries experiencing a 
measles outbreak in the African region

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// measles_plot_overall \\\
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 
 This section plots a bar chart for cases over time, filled by CFR
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 

```{r measles_plot_overall}
...
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// measles_plot_countries \\\
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 
 This section plots a bar chart for cases over time, filled by country
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 

```{r measles_plot_countries}

## Prepare data for plotting ---------------------------------------------------

measles_weekly_country <- measles_outbreak_data %>% 
  group_by(epiweek, country) %>% 
  summarise(cases_total = sum(new_suspected_cases, na.rm = TRUE) + 
              sum(new_probable_cases, na.rm = TRUE) + 
              sum(new_confirmed_cases, na.rm = TRUE),
...
ggplot(measles_weekly_country, aes(x = epiweek, y = cases_total, fill = reorder(country, cases_total))) + 
  geom_bar(stat = "identity") + 
  scale_fill_discrete_sequential(palette = "viridis",
                                  na.translate = FALSE, 
                                  name = "Country") + 
...
**Figure 14**: Spatial distribution of cumulative measles cases and CFR by country
in the African region as of `r format(aweek::week2date(end_week) + 7, "%d/%m/%Y")`
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 This section plots a map for countries experiencing a measles outbreak, including
 dots for cases and deaths as well as chloropleth for CFR
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 This section calculates counts and proportions fopr indicators used in 
 reporting events that are occuring due to Polio based on outbreak linelist
 data. 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 

```{r polio_events}

## subset outbreak data set for Polio
polio_outbreak_data <- outbreak_data %>% 
  filter(outbreak == "Polio/ Acute Flaccid Paralysis") 

## pull together countries experiencing polio outbreak
polio_countries <- str_c(
  unique(polio_outbreak_data$country),
  collapse = ", ")

## get summary counts for african region 
polio_counts <- polio_outbreak_data %>% 
  summarise(cases_total = sum(new_suspected_cases, na.rm = TRUE) + 
              sum(new_probable_cases, na.rm = TRUE) + 
              sum(new_confirmed_cases, na.rm = TRUE), 
...
Among the `r nrow(outbreak_data)` ongoing outbreaks in the African region, 
`r fmt_count(outbreak_data, outbreak == "Polio/ Acute Flaccid Paralysis")` are 
due to polio outbreaks. 
`r length(unique(polio_outbreak_data$country))` countries 
are experiencing a polio outbreak: `r polio_countries`. 

During `r str_glue("{current_month} {current_year}")`, 
`r polio_counts$cases_total` cases have been reported 
(`r polio_counts$cases_susp_prob` suspected and `r polio_counts$cases_conf` 
confirmed) including `r polio_counts$deaths` deaths (case fatality ratio : `r polio_counts$CFR`%). 

**Figure 15**: Distribution of cases and CFR by week for 
...
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// polio_plot_countries \\\
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 
 This section plots a bar chart for cases over time, filled by country
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 

```{r polio_plot_countries}

## Prepare data for plotting ---------------------------------------------------

polio_weekly_country <- polio_outbreak_data %>% 
  group_by(epiweek, country) %>% 
  summarise(cases_total = sum(new_suspected_cases, na.rm = TRUE) + 
              sum(new_probable_cases, na.rm = TRUE) + 
              sum(new_confirmed_cases, na.rm = TRUE),
...
ggplot(polio_weekly_country, aes(x = epiweek, y = cases_total, fill = reorder(country, cases_total))) + 
  geom_bar(stat = "identity") + 
  scale_fill_discrete_sequential(palette = "viridis",
                                  na.translate = FALSE, 
                                  name = "Country") + 
...
**Figure 17**: Spatial distribution of cumulative polio cases and CFR by country
in the African region as of `r format(aweek::week2date(end_week) + 7, "%d/%m/%Y")`
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 This section plots a map for countries experiencing a polio outbreak, including
 dots for cases and deaths as well as chloropleth for CFR
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 This section calculates counts and proportions fopr indicators used in 
 reporting events that are occuring due to yellow fever based on outbreak linelist
 data. 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 

```{r yellowfever_events}

## subset outbreak data set for yellow fever
yellowfever_outbreak_data <- outbreak_data %>% 
  filter(outbreak == "Yellow Fever") 

## pull together countries experiencing yellow fever outbreak
yellowfever_countries <- str_c(
  unique(yellowfever_outbreak_data$country),
  collapse = ", ")

## get summary counts for african region 
yellowfever_counts <- yellowfever_outbreak_data %>% 
  summarise(cases_total = sum(new_suspected_cases, na.rm = TRUE) + 
              sum(new_probable_cases, na.rm = TRUE) + 
              sum(new_confirmed_cases, na.rm = TRUE), 
...
Among the `r nrow(outbreak_data)` ongoing outbreaks in the African region, 
`r fmt_count(outbreak_data, outbreak == "Yellow Fever")` are 
due to yellow fever outbreaks. 
`r length(unique(yellowfever_outbreak_data$country))` countries 
are experiencing a yellow fever outbreak: `r yellowfever_countries`. 

During `r str_glue("{current_month} {current_year}")`, 
`r yellowfever_counts$cases_total` cases have been reported 
(`r yellowfever_counts$cases_susp_prob` susepcted and
`r yellowfever_counts$cases_conf` confirmed) including
`r yellowfever_counts$deaths` deaths (case fatality ratio : `r yellowfever_counts$CFR`%). 

**Figure 18**: Distribution of cases and CFR by week for 
`r length(unique(yellowfever_outbreak_data$country))` countries experiencing a 
yellow fever outbreak in the African region

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// yellowfever_plot_overall \\\
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 
 This section plots a bar chart for cases over time, filled by CFR
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 

```{r yellowfever_plot_overall}
...
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// yellowfever_plot_countries \\\
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 
 This section plots a bar chart for cases over time, filled by country
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 

```{r yellowfever_plot_countries}

## Prepare data for plotting ---------------------------------------------------

yellowfever_weekly_country <- yellowfever_outbreak_data %>% 
  group_by(epiweek, country) %>% 
  summarise(cases_total = sum(new_suspected_cases, na.rm = TRUE) + 
              sum(new_probable_cases, na.rm = TRUE) + 
              sum(new_confirmed_cases, na.rm = TRUE),
...
ggplot(yellowfever_weekly_country, aes(x = epiweek, y = cases_total, fill = reorder(country, cases_total))) + 
  geom_bar(stat = "identity") + 
  scale_fill_discrete_sequential(palette = "viridis",
                                  na.translate = FALSE, 
                                  name = "Country") + 
...
**Figure 20**: Spatial distribution of cumulative yellow fever cases and CFR by country
in the African region as of `r format(aweek::week2date(end_week) + 7, "%d/%m/%Y")`
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 This section plots a map for countries experiencing a yellow fever outbreak, including
 dots for cases and deaths as well as chloropleth for CFR
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ --> 
