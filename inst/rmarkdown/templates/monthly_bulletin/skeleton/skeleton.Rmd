...
for (i in loop_weeks) {
  ...
  afro_shp[[thresh_var]] <- cut(afro_shp[[thresh_var]],
  ...
  ## create categorical variable out of threshold_counts for week
  afro_shp[[disease_var]] <- cut(afro_shp[[disease_var]],
...
overall_plot <- ggplot() +
  ...
  scale_size_continuous(range = c(3,10), 
                        name = "Number of reports") +
  ## set the colours of dots 
  scale_colour_manual(values = c("#F7C252", "Dark red"), 
                      name = "") +
  ## adjust the size of dots for legend (colours only)
  guides(colour = guide_legend(override.aes = list(size = 4))) +
  ## add a scale bar
...
(`r measles_counts$cases_susp_prob` and `r measles_counts$cases_conf`) including
`r measles_counts$deaths` (case fatality ratio : `r measles_counts$CFR`%). 
...
(`r polio_counts$cases_susp_prob` and `r polio_counts$cases_conf`) including
`r polio_counts$deaths` (case fatality ratio : `r polio_counts$CFR`%). 
...
(`r yellowfever_counts$cases_susp_prob` and `r yellowfever_counts$cases_conf`) including
`r yellowfever_counts$deaths` (case fatality ratio : `r yellowfever_counts$CFR`%). 
